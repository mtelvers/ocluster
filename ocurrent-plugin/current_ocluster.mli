...
module Make () (_ : sig
  ...
  val v :
    ...
    unit ->
    t
  (** [v ()] is a builder with default settings.
  ...
  (** [with_timeout x t] is a copy of [t] with the specified timeout. *)

  val with_push_auth : (string * string) option -> t -> t
  (** [with_push_auth x t] is a copy of [t] with the specified push settings. *)

  val with_secrets : (string * string) list -> t -> t
  (** [with_secrets x t] is a copy of [t] with the specified secrets. *)
  ...
  val build :
    ...
    unit Current.t
  (** [build t ~pool ~src ~options dockerfile] builds [dockerfile] in context [src] using pool [pool].
  ...
  val build_obuilder :
    ...
    unit Current.t
  (** [build_obuilder t ~pool ~src spec] builds [spec] in context [src] using pool [pool].
      Note: all commits in [src] must be in the same repository. *)
